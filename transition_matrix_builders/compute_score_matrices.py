...
def load_rankings(path: Path | None) -> Dict[str, float]:
    ...
    for _, row in df.iterrows():
        ...
        if not ranks:
            continue
        rank_val = float(ranks[0])
        mapping[normalize_name(row[name_col])] = rank_val
    return mapping
...
def weight_for_row(row: pd.Series, player_map: Dict[str, tuple[str, str]], rank_map: Dict[str, float]) -> float:
    ...
    if rank is None:
        return 1.0
    # Bin weights: tougher opponents down-weighted
    if rank <= 10:
        return 0.5
    if rank <= 25:
        return 0.6
...
def build_score_transitions(df: pd.DataFrame, player_map: Dict[str, tuple[str, str]], rank_map: Dict[str, float]) -> pd.DataFrame:
    term = df[df["is_terminal"].apply(coerce_bool)]
    term = term[["match_id", "player_label", "point_score", "point_score_after"]].dropna()
    if term.empty:
        return term
    term = term.rename(columns={"point_score": "state", "point_score_after": "new_state"})
    term["weight"] = term.apply(lambda r: weight_for_row(r, player_map, rank_map), axis=1)
    counts = term.groupby(["state", "new_state"])["weight"].sum().reset_index(name="count")
    return counts
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute score-only transition matrix from state_transitions CSV."
    )
    parser.add_argument("--input", required=True, help="Path to state_transitions CSV.")
    parser.add_argument("--output_dir", default="score_matrices", help="Output directory.")
    parser.add_argument(
        "--per_player",
        action="store_true",
...
def main(argv: list[str]) -> int:
    ...
    if args.per_player:
        ...
        for p in players:
            ...
            transitions = build_score_transitions(sub, player_map, rank_map)
            ...
            print(f"Wrote score transitions ({len(transitions)} rows) and matrix to {out_dir}")
        return 0

    transitions = build_score_transitions(df, player_map, rank_map)
